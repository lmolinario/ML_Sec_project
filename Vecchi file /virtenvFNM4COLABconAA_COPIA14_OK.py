    ...
    try:
        ...
        for idx, model in enumerate(models):
            try:
                ...
                print(f"Errore durante l'attacco su {model_names[idx]}: {e}")
        ...
        try:
            with open(file_path, 'rb') as f:
                ...
                if isinstance(results, list):
                    ...
                    print(f"Attenzione: il formato dei dati caricati da '{file_path}' non è valido.")
...
def show_image(fig, local_idx, img, img_adv, expl, label, pred):
    ...
    try:
        ...
        print(f"Errore durante la visualizzazione delle immagini: {e}")
        ...
        for sample_id in range(num_samples):
            ...
            for idx, model in enumerate(models):
                try:
                    ...
                    print(f"Errore durante l'attacco sul modello \"{model_names[idx]}\" per il campione {sample_id}: {e}")
    ...
    try:
        for model_id, model_name in enumerate(model_names):
            ...
            if adv_ds is None or y_adv is None or attributions is None:
                print(f"Dati mancanti per il modello {model_name}. Passo al successivo.")
                continue
        ...
        try:
            ...
            for model_id, model_name in enumerate(model_names):
                try:
                    ...
                    print(f"Errore nell'elaborazione della confidence per il modello {model_name}, Sample {sample_id + 1}: {e}")
...
def analizza_campioni_discordanti(mismatched_samples, model_names, confidence_AA, confidence_FMN, y_pred_AA,
                                  y_pred_FMN):
    """
    Analizza i campioni discordanti tra AutoAttack e FMN e stampa le relative informazioni.

    Args:
        mismatched_samples (dict): Dizionario contenente i campioni discordanti per ogni modello.
        model_names (list): Lista dei nomi dei modelli.
        confidence_AA (dict): Dizionario delle confidence per AutoAttack.
        confidence_FMN (dict): Dizionario delle confidence per FMN.
        y_pred_AA (dict): Predizioni dei modelli sotto AutoAttack.
        y_pred_FMN (dict): Predizioni dei modelli sotto FMN.
    """
    ...
    for model_name, indices in mismatched_samples.items():
        ...
        print(f"\nAnalisi dei campioni discordanti per il modello {model_name}")
        print("-" * 90)
        ...
        for sample_data in indices:
            ...
            print(f"- Campione {idx}: \n  Confidenza AA={conf_AA.item()}, Confidenza FMN={conf_FMN.item()}")
            print(
            ...
            if label_AA == label_original and label_FMN == label_original:
                print("  * Nessuno dei due attacchi ha avuto effetto: il modello potrebbe essere molto robusto.")
            ...
            if conf_AA != "N/A" and conf_AA > conf_FMN:
                ...
                print(
                    "  * Le confidence sono simili, il modello potrebbe essere resistente a entrambi gli attacchi.")
...
def plot_perturbazioni(models, model_name_to_idx, dataset_labels, ts, results_AA_data, results_FMN_data,
                       Perturbation_Linf, Confidence_AA_4_sample, Confidence_FMN_4_sample, input_shape,
                       max_samples=3):
    """
    Genera e salva i plot delle perturbazioni per i modelli, confrontando gli attacchi AA e FMN.
    """
    for model_name, indices in Perturbation_Linf.items():
        ...
        for idx in list(indices.keys()):
            ...
            if attack_AA_success != attack_FMN_success:
                ...
                plt.savefig(f'results/Adv_{model_name}_sample{idx}.png')
...
if __name__ == "__main__":
    ...
    if not results_FMN_data:  # Se il caricamento non ha avuto successo, esegui l'attacco FMN
        print(f"Il file '{results_file_FMN}' non esiste o è corrotto. Generando nuovi risultati...")
        results_FMN_data = [
            {'model_name': name,
    ...
    if not results_AA_data:  # Se il file non esiste, esegui l'attacco AA
        print(f"Il file '{results_file_AA}' non esiste o è corrotto. Generando nuovi risultati...")
        results_AA_data = [
            {'model_name': name,
    ...
    print ('\nAvvio del calcolo della Confidenza in corso...\n')
    print("-" * 90)
    for model_idx, model in enumerate(models):
        ...
        if scores_AA is None or scores_AA.shape[0] == 0:
            print(f"Errore: il modello {model_names[model_idx]} non ha generato predizioni valide!")
    ...
    print("-" * 90)
    print("\nConfidenza per AutoAttack calcolata con successo!\n")
    print("-" * 90)

#################################################################################
    print("\nIdentifying samples for which one attack succeeds while the other fails\n!")
    ...
    for model_idx, model_name in enumerate(model_names):
        ...
        print(f"Analizzando il modello: {model_name:<40} - Campioni discordanti: {len(mismatched_samples[model_name])}")
    print("-" * 90)

    #################################################################################

    Confidence_AA_4_sample, Confidence_FMN_4_sample = analizza_campioni_discordanti(mismatched_samples, model_names, confidence_AA, confidence_FMN, y_pred_AA, y_pred_FMN)
    ...
    plot_perturbazioni(
        ...
        max_samples=3  # Mostra solo 3 esempi per modello
    )
