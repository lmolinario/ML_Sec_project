...
#labels.X = normalizer.transform(labels.X[:64, :])  # Normalizza i primi 64 campioni
#labels.Y = labels.Y[:64]  # Aggiorna le etichette corrispondenti
ts.X = normalizer.transform(ts.X)# Select first 64 test samples
#labels = labels[:64, :]  # Select first 64 test samples
ts = ts[:64, :]
