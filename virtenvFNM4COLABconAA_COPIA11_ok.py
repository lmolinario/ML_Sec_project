...
if __name__ == "__main__":
    ...
    for model_idx, model_name in enumerate(model_names):
        ...
        y_true = results_FMN_data[model_idx]['result']['adv_ds'].Y.tondarray()  # Usa y_true da results_FMN
        ...
        # Salvataggio dei campioni discordanti
    ...
    for model_name, indices in mismatched_samples.items():
        ...
        for idx in indices:
            ...
            # Nome della classe predetta dopo AutoAttack
            label_AA = dataset_labels[y_pred_AA[idx].item()]

            # Nome della classe predetta dopo FMN
            label_FMN = dataset_labels[y_pred_FMN[idx].item()]
            if y_pred_AA[idx] == y_true[idx] and y_pred_FMN[idx] == y_true[idx]:
                print(f"⚠️  Attenzione: il campione {idx} non è stato alterato da nessuno dei due attacchi.")
    ...
    # Dizionario per salvare la perturbazione L∞
    Perturbation_Linf = {}
    ...
    for model_name, indices in mismatched_samples.items():
        ...
        for idx in indices:
            # Campioni originali e avversari per i due attacchi
            ...
            # Calcolo della norma L∞ tra originale e adversarial
            ...
            if linf_AA < linf_FMN:
                print("  ✅ AutoAttack è più efficace con minore perturbazione.")
            elif linf_FMN < linf_AA:
                print("  ✅ FMN è più efficace con minore perturbazione.")
            else:
                print("  ⚠️ Entrambi gli attacchi hanno la stessa perturbazione L∞.")
    ...
    for model_name, indices in Perturbation_Linf.items():
        ...
        for idx in list(indices.keys()):
            ...
            conf_AA = float(
                Confidence_AA_4_sample[model_name][idx].item()) if model_name in Confidence_AA_4_sample and idx in \
                                                                   Confidence_AA_4_sample[model_name] else None
            conf_FMN = float(
                Confidence_FMN_4_sample[model_name][idx].item()) if model_name in Confidence_FMN_4_sample and idx in \
                                                                    Confidence_FMN_4_sample[model_name] else None
            ...
            if np.max(perturbation_AA) > 0:
                perturbation_AA = perturbation_AA / np.max(perturbation_AA)
            else:
                perturbation_AA = np.zeros_like(perturbation_AA)

            if np.max(perturbation_FMN) > 0:
                perturbation_FMN = perturbation_FMN / np.max(perturbation_FMN)
            else:
                perturbation_FMN = np.zeros_like(perturbation_FMN)
            ...
            if attack_AA_success != attack_FMN_success:
                ...
                # Immagine avversaria AA
                axes[0, 1].imshow(x_adv_AA.transpose(1, 2, 0))
                axes[0, 1].set_title(
                    f"Avversaria AA ({label_AA})\nConf: {conf_AA:.4f}" if conf_AA is not None else f"Avversaria AA ({label_AA})\nConf: N/A")
                axes[0, 1].axis("off")

                # Immagine avversaria FMN
                axes[0, 2].imshow(x_adv_FMN.transpose(1, 2, 0))
                axes[0, 2].set_title(
                    f"Avversaria FMN ({label_FMN})\nConf: {conf_FMN:.4f}" if conf_FMN is not None else f"Avversaria FMN ({label_FMN})\nConf: N/A")
                axes[0, 2].axis("off")

                axes[1, 0].text(0.5, 0.5, "Info Attacco", fontsize=12, ha='center', va='center')
                axes[1, 0].axis("off")
